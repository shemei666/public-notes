...
- **Induced Norm Structure**:
  ...
  **Properties of the norm:**
  ...
  2. $\|\lambda x\| = |\lambda| \|x\| \quad \forall \lambda \in \mathbb{R}$ _(Homogeneity)_
...
_(Note: $f_i = \pi_i \circ f$, where $\pi_i : \mathbb{R}^n \to \mathbb{R}$ is the $i$-th projection map)._
...
2. **Exponential of Commuting Matrices**: Show that if $A, B \in M_n(\mathbb{R})$ commute ($AB = BA$), then:
   $$ \exp(A + B) = \exp(A) \exp(B) $$
   _(Hint: Use the Cauchy product for power series)._
...
\usepackage{tikz-cd}
\begin{tikzcd}
M_n(\mathbb{R}) \arrow[r, "\exp"] \arrow[d, "\mathrm{trace}"'] & G_n^+(\mathbb{R}) \arrow[d, "\det"] \\
