...
Also since $S$ is a compact set we have that $\vec{y_{n}}$ has a convergent subsequence, let $y_{n_{k}}$ be the convergent subsequence. Also see that the projection of any point in the ray $R_{K}(x_{n})$ will have the same image under the projection.
